...
if (isset($_GET['e2'])) {
    ...
    foreach ($user->categories as $category) {
        foreach ($category->streams as $stream) {
            if ($stream->running == 1) {
                echo "#SERVICE 1:0:1:0:0:0:0:0:0:0:http%3A//" . $setting->webip . "%3A" . $setting->webport . "/playlist/" . $user->username . "/" . $user->password . "/" . $stream->id . "\r\n";
    ...
    foreach ($user->categories as $category) {
        foreach ($category->streams as $stream) {
            ...
            if ($stream->running == 1) {
                ...
                echo "http://" . $setting->webip . ":" . $setting->webport . "/playlist/" . $user->username . "/" . $user->password . "/" . $stream->id . "\r\n";
            }
        }
    }
    header('Content-Type: application/octet-stream');
    ...
    foreach ($user->categories as $category) {
        foreach ($category->streams as $stream) {
            ...
            if ($stream->running == 1) {
                ...
                echo "<HQ>http://" . $setting->webip . ":" . $setting->webport . "/playlist/" . $user->username . "/" . $user->password . "/" . $stream->id . "</HQ>\r\n";
